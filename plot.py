...
from Tkinter import *
from Canvas import Line, CanvasText, Polygon
import string, math
from pUtils import *
from math import pi
...
class GraphPoints:
    ...
    def fitToScale(self, scale=(1,1), shift=(0,0)):
        ...
        for x,y in self.points:
            self.scaled.append(((scale[0]*x)+shift[0],(scale[1]*y)+shift[1]))
...
#
# Data must consist of two points. A rectangle shall be drawn in these points.
#
class GraphBoxes(GraphPoints):
    def __init__(self, points, **attr):
        GraphPoints.__init__(self, points, attr)
    ...
    def draw(self, canvas):
        ...
        for i in range(len(self.points)/2):
            ...
            canvas.create_rectangle(arguments,fill=color,width=width)

class GraphHistogram(GraphPoints):
    def __init__(self, points, **attr):
        GraphPoints.__init__(self, points, attr)
...
class GraphBase(Frame):
    ...
    def plot(self):
        self.canvas.postscript(file='kamran.eps',pagewidth=600,pageheight=800);
...
#    boxes = GraphBoxes(data,color='blue')
#    graphObject = GraphObjects([boxes,])
