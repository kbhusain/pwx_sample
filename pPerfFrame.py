...
class MyPerfFileNodes(Tree.Node):
...
class framePerfFileParameters(Frame):
	"""
	...
	def __init__(self,master,rateObject=None):
		...
		self.btnSaveWell = Button(self.frm_details,text="Save Selected Perf", command=self.f_savePerf)
	...
	def mapObjToGui(self,perfObject,forceTheIssue=0):
		...
		if self.m_treeBrowser == None: 
			self.m_treeBrowser = Tree.Tree(master=self.frm_tree, 
				root_id = os.sep,  root_label='Perf Data', 
	...
	def f_savePerf(self):
		...
		if self.lastPerf == None: 
			if self.lastWell == None: 
				...
				return 
			reply = askyesno('Whoa!',"Are you sure you want to set the non-empty entries for all Perfs in this well?")
			if reply == True: 
